...
$startMenuShortcut = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\CommanDOS.lnk"
...
$szmelcCmdPath = "$env:WINDIR\System32\szmelc.cmd"
...
function Enable-ExecutionPolicy {
    try {
        ...
        Write-Host "⚠️ Could not set ExecutionPolicy. Try running as Administrator." -ForegroundColor Red
    }
}

# ───── Add szmelc Global Command ────────────────────────────────────
function Add-GlobalCommand {
    $cmdContent = "@echo off`r`nPowerShell -NoProfile -ExecutionPolicy Bypass -Command ""Start-Process PowerShell -Verb RunAs -ArgumentList '-NoProfile','-ExecutionPolicy Bypass','-File `"$destPath\main.ps1`'""""
    Set-Content -Path $szmelcCmdPath -Value $cmdContent -Encoding ASCII -Force
    Write-Host "💻 'szmelc' command added globally" -ForegroundColor Green
}

# ───── Pin to Start Menu and Taskbar ────────────────────────────────
function Pin-ToStartAndTaskbar {
    Copy-Item $desktopShortcut $startMenuShortcut -Force
    ...
    try {
        ...
        Write-Host "📌 Pinned to Taskbar and Start Menu." -ForegroundColor Green
    } catch {
        Write-Host "⚠️ Pinning to taskbar failed. Might require English locale or admin." -ForegroundColor Yellow
    }
}
...
function Install-Or-Update {
    ...
    Add-GlobalCommand
    Pin-ToStartAndTaskbar

    Write-Host "`n✅ CommanDOS installed to C:\CommanDOS"
    Write-Host "📎 Shortcut added to Desktop, Start Menu, and Taskbar.`n"
}
...
function Uninstall-CommanDOS {
    ...
    if (Test-Path $szmelcCmdPath) {
        Remove-Item $szmelcCmdPath -Force
        Write-Host "🗑️ Removed 'szmelc' command"
    }
